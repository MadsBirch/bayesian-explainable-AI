...
# #### Build model
MAP_model = big_daddy(in_channels=1, n_cats=7).cpu()

def get_model_metrics(n_iter = 10, hessian_structure = 'full'):
    METRICS_DICT={
    ...
    for i in range(n_iter):
        ...
        MAP_model.load_state_dict(torch.load(PATH,  map_location = 'cpu'))
        MAP_model.eval()
        ...
        for j, m in enumerate(model_list):
            ...
            for inputs, labels in tqdm(val_loader, total=len(val_loader)):
                ...
                lab_idx = torch.argmax(labels, dim=1)

                preds = torch.argmax(F.softmax(outputs, dim = 1),dim=1)
                running_corrects += torch.sum(preds == lab_idx)
                F1s.append(f1_score(lab_idx, preds, average = 'weighted'))

            predictions = torch.cat(predictions)
            ACC_LIST[j,i] = (running_corrects/len(val_loader))
    ...
    METRICS_DICT['MA']['mean_ACC'] = np.mean(ACC_LIST[1,:])
...
for h in hessian_structures:
    METRICS_DICT = get_model_metrics(n_iter = 10, hessian_structure = h)

    # SAVE
    with open(h +'_DICT.pkl', 'wb') as pkl_handle:
